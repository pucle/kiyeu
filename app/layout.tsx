import type { Metadata } from 'next';
import './globals.css';

export const metadata: Metadata = {
  metadataBase: new URL('https://kiyeu-jf8v.vercel.app'),
  title: 'fen li rì mai đờ của Púc',
  description: 'Bạn có hẹn với Púc vào ngày 9/5/2026 tại THPT Chuyên Lê Quý Đôn, Đà Nẵng lúc 11:00 - 13:30',
  openGraph: {
    title: 'fen li rì mai đờ của Púc',
    description: 'Bạn có hẹn với Púc vào ngày 9/5/2026 tại THPT Chuyên Lê Quý Đôn, Đà Nẵng lúc 11:00 - 13:30',
    url: '/',
    siteName: 'fen li rì mai đờ của Púc',
    images: [
      {
        url: '/preview.png',
        width: 1200,
        height: 630,
        alt: 'fen li rì mai đờ của Púc',
      },
    ],
    locale: 'vi_VN',
    type: 'website',
  },
  twitter: {
    card: 'summary_large_image',
    title: 'fen li rì mai đờ của Púc',
    description: 'Bạn có hẹn với Púc vào ngày 9/5/2026 tại THPT Chuyên Lê Quý Đôn, Đà Nẵng lúc 11:00 - 13:30',
    images: ['/preview.png'],
  },
};

export default function RootLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <html lang="vi">
      <head>
        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="anonymous" />
        <link
          href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700&family=Crimson+Pro:ital,wght@0,400;0,500;0,600;0,700;1,400&family=Courier+Prime:wght@400;700&display=swap"
          rel="stylesheet"
        />
      </head>
      <body>{children}</body>
    </html>
  );
}
